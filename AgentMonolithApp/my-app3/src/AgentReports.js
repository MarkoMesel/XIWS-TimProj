...
class AgentReports extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('https://localhost:8080/schedule/reservations/report/pending', axiosConfig)
            .then(response => {
    ...
    handleReport = (event) => {
        ...
        if (this.state.mileage !== null) {
            axios.post(
                'https://localhost:8080/schedule/reservations/report', reportData, axiosConfig
            ).then(response => {
                if (response.status === 200) {
