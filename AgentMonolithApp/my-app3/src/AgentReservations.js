...
class AgentReservations extends Component {
 ...
 componentDidMount(){
    ...
    axios.get('https://localhost:8080/schedule/reservations/publisher',axiosConfig)
    .then(response => {
 ...
 handleApprove = (event) => {
    let approveLink = 'https://localhost:8080/schedule/bundle/' + event.target.value + '/approve';
    let body = null;
 ...
 handleReject = (event) => {
    let rejectLink = 'https://localhost:8080/schedule/bundle/' + event.target.value + '/reject';
    let body = null;
