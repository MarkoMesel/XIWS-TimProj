...
public class CarProvider {
	...
	public SoapResponse addCar(SoapAddCarRequest request) {
		...
		if(token.getRoleId()==1) {
			List<CarDbModel> cars = unitOfWork.getCarRepo().findByPublisherIdAndPublisherTypeId(request.getPublisherId(), request.getPublisherTypeId());
			if(cars.size()>=3) {
	...
	public SoapSearchCarsResponse seachCars(SoapSearchCarsRequest request) {
		...
		List<CarDbModel> cars =	unitOfWork.getCarRepo()
				.findAll()
				.stream()
				.filter(car -> car.getLocation().getId() == request.getLocationId())
		...
		for (CarDbModel objectIn : cars) {
			...
			SoapCarPriceResponse carPriceResponse = scheduleServiceClient.getCarPrice(objectIn.getId(), request.getStartDate(), request.getEndDate());
			if (!carRatingResponse.isSuccess()) {
				response = new SoapSearchCarsResponse();
				response.setSuccess(false);
				return response;
	...
	public SoapResponse deleteCarImage(SoapDeleteImageRequest request) {
		...
		if(!jwtUtil.isAutharized(tokenParseResult, 2, image.get().getCar().getId(), image.get().getCar().getPublisherType().getId())) {
			response.setAuthorized(true);
			return response;
		}
